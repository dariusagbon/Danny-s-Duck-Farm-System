<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">Add New Customer</h2>
    </x-slot>
                ...
                <form action="{{ route('customers.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select name="customer_type" id="customer_type" required class="mt-2 block w-full rounded-3xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                                ...
                                <option value="retail" @selected(old('customer_type') === 'retail')>Retail</option>
                                <option value="wholesale" @selected(old('customer_type') === 'wholesale')>Wholesale</option>
                                <option value="restaurant" @selected(old('customer_type') === 'restaurant')>Restaurant</option>
                            </select>
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="credit_limit" id="credit_limit" value="{{ old('credit_limit') }}" class="mt-2 block w-full rounded-3xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4">
                        <button type="submit" class="inline-flex items-center justify-center rounded-3xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-500">Save Customer</button>
